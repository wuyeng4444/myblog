...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: '关于', href: '/about' },
    { name: '设定集', href: 'https://wuyeng4444.github.io/-/' },
  ];
  ...
  return (
    <>
      {/* PC端导航栏 */}
      <header className={`hidden md:block w-full fixed top-0 left-0 right-0 z-50 transition-all duration-500 border-b ${showNav ? 'translate-y-0' : '-translate-y-full'} bg-white/40 dark:bg-slate-900/50 backdrop-blur-md border-white/20 dark:border-white/5 shadow-sm`}>
        <div className="w-[90%] max-w-6xl mx-auto h-16 flex items-center justify-between px-4 sm:px-[30px] box-border">
          ...
          <nav className="flex gap-4 lg:gap-6 xl:gap-8 text-sm font-bold">
            {/* PC端依然使用全量的 navLinks */}
            {navLinks.map((link) => {
              const isActive = !isExternalLink(link.href) && (pathname === link.href || pathname === `${link.href}/`);
              ...
              if (isExternalLink(link.href)) {
                return (
                  <a key={link.href} href={link.href} target="_blank" rel="noopener noreferrer" className={cls}>
                    {link.name}
                    <span className="ml-0.5 text-[10px] text-indigo-500 dark:text-indigo-400">↗</span>
                  </a>
                );
              }
              return (
              ...
              >
                ...
                >
                  ...
                  {mobileNavLinks.map((link, index) => {
                    ...
                    const btnCls = `flex items-center justify-center w-full h-full rounded-full transition-all duration-300 ${
                      isActive
                      ...
                      >
                        <motion.div style={{ rotate: inverseRotation }} className="w-full h-full">
                          {isExternalLink(link.href) ? (
                            <a href={link.href} target="_blank" rel="noopener noreferrer" onClick={() => setIsMobileMenuOpen(false)} className={btnCls}>
                              <span className="text-[11px] font-black">{link.name}</span>
                            </a>
                          ) : (
                            <Link href={link.href} onClick={() => setIsMobileMenuOpen(false)} className={btnCls}>
                              <span className="text-[11px] font-black">{link.name}</span>
                            </Link>
                          )}
                        </motion.div>
